...
public class FFT
{
     ...
     * @param signal the real signal for which to compute the power spectrum.
     * @return the power spectrum, as an array of length N/2 (where N is the power of two
     ...
     * @param fft the array of real and imag parts of the complex number array,
     * fft[0] = real[0], fft[1] = real[N/2], fft[2*i] = real[i], fft[2*i+1] = imag[i] for 1<=i<N/2
     * @return an array of length real.length/2 containing numbers 
     ...
     * is not a power of two. 
     * @param signal the real signal for which to compute the power spectrum.
     * @return the log amplitude spectrum, as an array of length N/2 (where N is the power of two
     * greater than or equal to signal.length):
     ...
     * r[i] = sqrt(real[i]*real[i] + imag[i]*imag[i])
     */
     ...
     * 
     * @param real in "forward" FFT: as input=the time-domain signal to transform, as output=the real part of the
     * complex frequencies; in inverse FFT: as input=the real part of the complex frequencies, as output=
     * the time-domain signal.
     * @param imag in "forward" FFT: as input=an empty array, as output=the imaginary part of the
    ...
    public static void realTransform(double data[], boolean inverse)
    {
        ...
        if (inverse) twoPi = MathUtils.TWOPI;
    ...
    public static double[] autoCorrelate(final double[] signal)
    {
        if (signal == null)
            throw new NullPointerException("Received null argument");
        int N = 2*2*signal.length;
        
        assert MathUtils.isPowerOfTwo(N);
        double[] Signal = new double[signal.length];
        double [] fft = new double[N];
    	System.arraycopy(signal, 0, Signal, 0, signal.length);
...
//        for(int i =0; i<N/2;i++){
//        	System.out.println(fft[2*i]+"+"+fft[2*i+1]+"i");
//        }
    ...
    /**
     * Edited by Nathaniel Cruz
     * Compute the autocorrelation of a signal, by inverse transformation of its
     * power spectrum.
     * This method applies zero padding where necessary to ensure that the result
     * is not polluted because of assumend periodicity.
     * @param signal
     * @return the normalized correlated signal, of the same length as the input signal
     */
    public static double[] autoCorrelateWithZeroPadding(final double[] signal)
    {
        ...
        double[] result = new double[signal.length];
        for(int i=0; i<signal.length;i++){
        	result[i]=fftAutocorr[2*i]/result[0];;
        }
        
        return result;
    }
    
    /**Created by Nathaniel D. Cruz
     * Compute the envelope of the signal, by getting the hilbert transform, mulitply
     * with the conjugate and get the square root.
     * @param signal complex
     * @return envelope real
     *
     * 
    */
    public static double[] envelope(final double[] signal){
    	//int n = MathUtils.closestPowerOfTwoAbove(2*signal.length);
        int n = signal.length;
        
    	double[] fftSignal = new double[n];
        System.arraycopy(signal, 0, fftSignal, 0, signal.length);
        transform(fftSignal, false);
        ...
        transform(fftSignal, true);
       
        
     // Now multiply with complex conjugate in the frequency domain,
        // and save in fft:
        //fftSignal[0] = fftSignal[0] * fftSignal[0]; // because imag[0] is 0
        //fftSignal[1] = fftSignal[1] * fftSignal[1]; // and fftSignal[1] is actually real[N/2]
        for (int i=0; i<n; i+=2) {
            fftSignal[i] = fftSignal[i]*fftSignal[i] + fftSignal[i+1]*fftSignal[i+1];
            fftSignal[i+1] = 0;
        }

//        for(int i=0; i<fftSignal.length/2;i++){
//			System.out.println(fftSignal[2*i]+"+"+fftSignal[2*i+1]+"i");
//		}
        double[] result = new double[N];
        
        
		for (int i = 0; i < N; i++) {
			result[i] = Math.sqrt(fftSignal[2*i]);
...
//            
///*
//            double[] conv1 = autoCorrelateOrig(ar);
//            double[] conv2 = autoCorrelate(ar);
//            System.err.println("Difference between conv1 and 2: "+ MathUtils.sumSquaredError(conv1, conv2));
//            for (int j=0; j<conv1.length; j++) {
//                double ddelta = Math.abs(conv1[j]-conv2[j]);
//                //if (ddelta > 1.E-4) System.err.println("delta["+j+"]="+ddelta+" out of "+conv1[j]);
//            }
//*/
//            // Transform:
...
//            for (int j=0; j<N; j++) {
//                result1[2*j] = ar[j]; result1[2*j+1] = ai[j];
//            }
//            // Transform 2:
...
//                if (delta[j] > 1.E-4) System.err.println("delta["+j+"]="+delta[j]);
//            }
//            result3[1] = result2[N];
//            
//            // And backwards:
